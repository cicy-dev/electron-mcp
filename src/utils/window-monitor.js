...
const CAPTURE_DIR = path.join(os.homedir(), 'Desktop', process.env.TEST === 'TRUE' ? 'CaptureDataTest' : 'CaptureData');
  ...
  win.webContents.debugger.on('message', async (event, method, params) => {
    ...
    if (method === 'Network.loadingFinished') {
      ...
      try {
        ...
        fs.writeFileSync(filePath, result.base64Encoded ? result.body : content, 
