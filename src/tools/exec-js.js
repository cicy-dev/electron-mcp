  ...
  server.registerTool(
    ...
    async ({ win_id, code }) => {
      try {
        ...
        const wrappedCode = code.trim().startsWith('return') || code.includes(';') 
          ? `(async () => { try { ${code} } catch(e) { return 'Error: ' + e.message } })()`
          : `(async () => { try { return ${code} } catch(e) { return 'Error: ' + e.message } })()`;
